...
var Project = Backbone.Model.extend (/** @lends Project.prototype */{
	...
	getRsc: function (opts, cb) {
		...
		async.waterfall ([
			function getRsc (cb) {
				me.rm.getRsc ({session: session, rid: rid, filter: {id: id}, rscFormat: true}, function (err, opts) {
					cb (err, opts);
				});
			},
			function getAttrs (opts, cb) {
				if (rid == "class") {
					me.rm.getRsc ({session: session, rid: "classAttr", filter: {classId: id}, rscFormat: true, source: opts.source}, function (err, _opts) {
						opts.data = opts.data.concat (_opts.data);
						cb (err, opts);
					});
				} else
				if (rid == "query") {
					me.rm.getRsc ({session: session, rid: "queryAttr", filter: {queryId: id}, source: opts.source, rscFormat: true}, function (err, _opts) {
						opts.data = opts.data.concat (_opts.data);
						cb (err, opts);
					});
				} else
				if (rid == "object") {
					me.getRsc ({session: session, rid: rid, id: opts.data [0].data.classId}, function (err, _opts) {
						me.rm.getRsc ({session: session, rid: "objectAttr", filter: {objectId: id}}, function (err, _opts) {
